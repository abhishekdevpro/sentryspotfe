...
const InfoBadge = ({ label, value, icon: Icon }) => (
  <div className="bg-blue-50/20 rounded-lg p-3 sm:p-4 border border-gray-200 hover:shadow-sm transition-shadow">
    <div className="flex items-center gap-2 mb-2">
      {Icon && (
        <Icon className="h-3 w-3 sm:h-4 sm:w-4 text-blue-900 flex-shrink-0" />
      )}
      <p className="app-text-h3">{label}</p>
    </div>
    <p className="app-text-p break-words leading-tight">
      {value}
    </p>
  </div>
);
...
const StatusBadge = ({ label, value }) => (
  <div className="flex items-center justify-between p-2 sm:p-3 app-light-bg border border-gray-200 rounded-lg hover:shadow-sm transition-shadow">
    <span className="app-text-p pr-2">
      {label}
    </span>
    <span
...
const EducationCard = ({ education }) => (
  <div className="app-light-bg border border-blue-200 rounded-lg p-3 sm:p-4 shadow-sm hover:shadow-md transition-shadow">
    <div className="space-y-2 sm:space-y-3">
      <h5 className="app-text-h3 leading-tight">
...
const ExperienceCard = ({ experience }) => (
  <div className="app-light-bg border border-blue-200 rounded-lg p-3 sm:p-4 shadow-sm hover:shadow-md transition-shadow">
    <div className="space-y-2 sm:space-y-3">
      <div>
        <h5 className="font-semibold text-blue-900 text-sm sm:text-lg leading-tight">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-4 text-xs sm:text-sm">
        <div>
          ...
          <span className="ml-1 sm:ml-2 text-blue-900">
            {experience.time_period_start} -{" "}
            {experience.is_present === 1
              ? "Present"
...
const ProfileHeader = ({
  ...
  <div className="relative overflow-hidden">
    ...
    <div className="relative h-32 sm:h-40 md:h-48 lg:h-56 bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700">
      ...
      <div className="absolute top-5 right-5 sm:top-10 sm:right-10 w-16 h-16 sm:w-32 sm:h-32 app-light-bg/10 rounded-full blur-xl"></div>
    ...
    <div className="relative -mt-16 sm:-mt-20 md:-mt-24 px-2 sm:px-4 lg:px-8">
      <div className="app-light-bg rounded-xl sm:rounded-2xl shadow-xl border border-gray-200 p-3 sm:p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-6">
          {/* Enhanced Profile Picture */}
          <div className="relative group flex-shrink-0">
            <div className="relative rounded-full h-20 w-20 sm:h-24 sm:w-24 md:h-28 md:w-28 lg:h-32 lg:w-32 border-3 sm:border-4 border-white app-light-bg shadow-lg overflow-hidden">
              <img
                src={getProfilePhoto()}
                alt={getFullName()}
          ...
          <div className="flex-1 text-center md:text-left w-full">
            <div className="mb-3 sm:mb-4">
              <h2 className="app-text-h2 ">{getFullName()}</h2>
            ...
            <div className="flex flex-wrap justify-center sm:justify-start gap-1 sm:gap-2 mb-4 sm:mb-6">
              {userInfo?.current_location && (
                <span className="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                  ...
                  <span className="truncate max-w-[100px] sm:max-w-none app-text-sm">
                    {userInfo.current_location}
                  </span>
                </span>
              )}
              {userInfo?.work_experience_name && (
                <span className="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                  ...
                  <span className="truncate max-w-[120px] sm:max-w-none app-text-sm">
                    {userInfo.work_experience_name}
                  </span>
                </span>
              )}
        ...
        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200">
          <div className="flex items-center justify-center sm:justify-end text-xs sm:text-sm text-gray-500">
            <div className="flex items-center">
              ...
              <span className="app-text-p app-text-muted">Last seen: {getLastLoginDate()}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen app-light-bg ">
      ...
      <div className="px-2 sm:px-4 lg:px-8 py-4 sm:py-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          {userInfo?.email && (
            <InfoBadge icon={Mail} label="Email" value={userInfo.email} />
          )}
          {userInfo?.phone && (
            <InfoBadge icon={Phone} label="Phone" value={userInfo.phone} />
          )}
        </div>

        {/* Tabs */}
        <div className="app-light-bg rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-4 sm:mb-6">
          ...
          <div className="p-3 sm:p-4 lg:p-6">
                ...
                {userInfo?.resume ? (
                  ...
                  <div className="text-center py-8 sm:py-12">
                    ...
                    <p className="text-gray-400 text-xs sm:text-sm mt-1 sm:mt-2">
                      Please upload your resume to view it here.
                    </p>
                  </div>
                )}
              </div>
            ...
            {activeTab === "profile" && (
              <div className="space-y-6 sm:space-y-8">
                ...
                <div>
                  <h3 className="app-text-h2 flex items-center mb-4">
                    ...
                    <div className="mb-4 sm:mb-6">
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {userInfo.preferred_location.map((location, index) => (
                          <span
                            key={index}
                            className="inline-block bg-blue-100 text-blue-900 px-2 sm:px-3 py-1 rounded-full app-text-p"
                          >
                            {location}
                          </span>
                        ))}
                  ...
                  {userInfo?.job_applyer_skills?.length > 0 && (
                    <div className="mb-4 sm:mb-6">
                      <p className="app-text-h3 mb-2 sm:mb-3">
                        Skills
                      </p>
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {userInfo.job_applyer_skills.map((skill, index) => (
                          <span
                            key={index}
                            className="inline-block bg-blue-900 text-white px-2 sm:px-3 py-1 rounded-full app-text-p"
                          >
                            {skill}
                          </span>
                        ))}
                  ...
                  {userInfo?.additional_info?.languages && (
                    <div className="mb-4 sm:mb-6">
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {userInfo.additional_info.languages.map(
                          (language, index) => (
                            <span
                              key={index}
                              className="inline-block bg-gray-200 text-gray-800 px-2 sm:px-3 py-1 rounded-full app-text-p"
                            >
                ...
                {education.length > 0 && (
                  <div>
                    <h4 className="app-text-h3 mb-3 sm:mb-4">
                      Education
                    </h4>
                    <div className="space-y-3 sm:space-y-4">
                ...
                {userInfo?.additional_info && (
                  <div>
                    <h3 className="app-text-h3 mb-3 sm:mb-4">
                      Additional Information
                    </h3>
                    <div className="grid grid-cols-1 gap-2 sm:gap-3">
                      <StatusBadge
                        ...
                        value={
                          userInfo.additional_info.is_veteran_or_ex_military
                        }
                      />
                      <StatusBadge
                        label="Reasonable Adjustments Required"
                        value={
                          userInfo.additional_info.is_reasonable_adjustments
                        }
                      />
                      <StatusBadge
