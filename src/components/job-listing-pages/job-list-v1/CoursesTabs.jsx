...
const CoursesTabs = () => {
  ...
  const defaultTrainerImage =
    "https://placehold.co/100x100/2563eb/ffffff?text=Trainer";
    ...
    const fetchCourses = async () => {
      try {
        ...
        if (
          data &&
          data.data &&
          data.data.courseResponse &&
          Array.isArray(data.data.courseResponse)
        ) {
  ...
  const filteredCourses = Array.isArray(courses)
    ? courses.filter((course) => {
        ...
        console.log("Filtering:", {
          category,
          tabTitle,
          matches: category.includes(tabTitle),
        });
  ...
  return (
    <div className="p-2 sm:p-4 max-w-full ">
      ...
      {/* <div className="overflow-x-auto mx-auto">
        <div className="flex space-x-2 sm:space-x-4 mb-6 border-b-2 border-blue-900 min-w-max">
          {tabs.map((tab) => (
            <button
              key={tab.id}
      ...
      <div className="w-full overflow-x-auto">
        <div className="flex justify-center items-center space-x-4 sm:space-x-4 mb-6 min-w-max scrollbar-hide">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`relative px-3 sm:px-5 py-2 font-medium whitespace-nowrap text-sm sm:text-base transition-colors duration-200
          ${
            activeTab === tab.id
              ? "text-blue-600"
              ...
              <div className="p-6">
                ...
                <div className="flex items-center mb-3">
                  ...
                  <span className="text-gray-600">
                    {course.rating || 0} ({course.enrolled_jobseeker_count || 0}{" "}
                    students)
                  </span>
                </div>
